...
;(function($) {
    ...
    support.sticky = div.style.position === 'sticky';
    div.style.position = '';
	...
	window.trace = (function() {
		...
		function create() {
			if ( !box ) {
				...
				box.style.cssText = 'position:fixed;left:5px;top:5px;max-width:75%;font-family:verdana;font-size:9px;color:#000;line-height:1.35em;margin:0;padding:3px 3px 2px 38px;border:1px solid #000;background:#fff;display:none;z-index:10000;opacity:0.75;filter:alpha(opacity=75);';
				box.onclick = clear;
				addbox();
			}
		}
...
;(function($) {
	var u = navigator.userAgent, 
		...
		iOS = (navigator.platform=='MacIntel'&&navigator.maxTouchPoints>1)||(/iPhone|iPad|iPod/i).test(u), json, n=0;
	$.each($('scr'+'ipt[type="appli'+'cation/ld'+'+json"]'), function(i) {
		var fave = 'We are your fave. - www.fave.kr', json = {};
		try {
			json = eval('(['+$(this).text()+'])');
		} catch(e) {}
			...
			if (fave.encoding&&fave.encoding.length) {
				$.each(fave.encoding, function(k) {
					if (this.encodingFormat) {
						...
						} else if (this.encodingFormat.toLowerCase().indexOf('glt'+'f+json')>-1) {
							fave.gltf = contentUrl;
						} else if (this.encodingFormat.toLowerCase().indexOf('glt'+'f-binary')>-1) {
							fave.glb = contentUrl;
						}
					}
				});
			}
			fave['@type']=='3D'+'Model'&&(function(fave){
				var link = android?fave.glb||fave.gltf:iOS?fave.usdz:0;
				android&&link&&(link='inten'+'t://arvr.goo'+'gle.com/scene-vie'+'wer/1.0?file='
				.concat(link,'&mode=ar_only#Int'+'ent;scheme=https;package=com.goo'+'gle.ar.core;action=and'+'roid.inte'+'nt.action.VIEW;S.browser_fallback_url=')
				.concat(location.origin,';end;'));
				link&&$(fave['button']||'.__a'+'r__button').addClass('__'+'on').find('a.__li'+'nk').attr({href:link});
			})(fave);
		});
	});
})(window.jQuery);
;(function($) {
	...
	function xlider($box, option) {
		...
		var option = option || {},
			...
			noCenter = option.nocenter || false,
